...
function KTypeColor(Utype: any, TType: any, Uobj: any) {//(種別,その種別の背景色を変える)
    for (key in Uobj) {
...
function KJTypeColor(Utype: any, TType: any, Uobj: any) {//(種別,その種別の背景色を変える)
    for (key in Uobj) {
        if (Utype.startsWith(Uobj[key].type)) {
...
function JTypeColor(Utype: any, TType: any, Uobj: any) {//(種別,その種別の文字色を変える)
...
//博多駅限定
function J_hakataTypeColor(Utype: any, TType:any, Uobj:any) {//(種別,その種別の文字色を変える)
    //console.log(Uobj.Typea);
    for (key in Uobj) {
        if (Utype.startsWith(Uobj[key].type)) {
...
function JTypeIncludeColor(Utype: any, TType:any, Uobj:any) {//(種別,その種別の文字色を変える,'含む')
    for (key in Uobj) {
...
//JR四国
function allJRSIncludeColor(obj: any) {
    for (let ia = 0; ia < Tablenum; ia++) {
        for (let ib = 0; ib < Tablenums[ia]; ib++) {
