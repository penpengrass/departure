...
import { Sunport, Ishiduti, Uzushio, Uwakai, Shiokaze, Mrapid } from "./Shidetailset";
...
if (station == '高松駅') {
    ...
    for (var line = 0; line < iyonakayama.length; line++) {
        if (TrainNumber[0][0] == iyonakayama[line] || TrainNumber[1][0] == iyonakayama[line]) {
    ...
    for (var line = 0; line < iyohojo.length; line++) {
        if (TrainNumber[0][0] == iyohojo[line]) {
    ...
    for (var line = 0; line < takuma2.length; line++) {
        if (TrainNumber[0][0] == takuma2[line]) {
