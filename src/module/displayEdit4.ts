...
export function DesMiddle(td: number, tr: number, word: string) {
    ...
    if (matches[tr] && d_Tag) {
        ...
        var Des_HTML = '<span class="DesLeft" id="DesLeft' + (td + 1) + (tr + 1) + '">' + matches[tr][1]
            + '</span>' + '<span class="DesMiddle">' + word + '</span>' + '<span id="DesRight' + (td + 1) + (tr + 1) + '">' + matches[tr][2] + '</span>';
        ...
        } else if (DesRight !== null && station == '糸崎駅' && matches[tr][2].length > 2) {
            DesRight.style.display = 'inline-block';
            DesRight.style.transform = "scaleX(0.65)" + "translate(-25%,0%)";
        }
        if (DesLeft && matches[tr][1].length > 2) {
            //console.log(DesRight.textContent.length);
